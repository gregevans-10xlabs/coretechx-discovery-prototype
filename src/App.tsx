  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        <input className="flex-1 bg-white text-slate-800 rounded-lg px-3 py-2 text-sm border border-slate-300 focus:outline-none focus:border-[#00BDFE]"
          placeholder={placeholder||"Ask..."} value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==="Enter"&&ask()}/>
        <button onClick={ask} disabled={loading||!q.trim()} className="bg-[#00BDFE] hover:bg-[#0099d4] disabled:opacity-40 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors">Ask</button>
      </div>
    </div>
  );
}
